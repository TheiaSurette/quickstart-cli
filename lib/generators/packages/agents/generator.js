...
export class AgentRulesGenerator {
    ...
    async generateRootAgentFiles() {
        await Promise.all([
            fs.outputFile(path.join(this.projectDir, 'AGENTS.md'), generateAgentsMd(this.templateParams)),
            fs.outputFile(path.join(this.projectDir, 'CLAUDE.md'), generateClaudeMd(this.templateParams)),
        ]);
    }
