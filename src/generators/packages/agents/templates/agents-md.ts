...
export const generateAgentsMd = (params: AgentTemplateParams): string => {
  const { config, features } = params;

  const packages = [];
  if (features.ui) packages.push('ui/ (React + Vite)');
  if (features.api) packages.push('api/ (FastAPI + Python)');
  if (features.db) packages.push('db/ (PostgreSQL + SQLAlchemy)');
